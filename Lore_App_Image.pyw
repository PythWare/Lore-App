...
class LoreApp():
    ...
    def __init__(self, root):
        ...
        self.image_file = None # used for the image file
            ...
            if self.filename:
                ...
                with open(self.filename, "rb") as f1: # Open the Lore file
                    ...
                    for i in range(character_count): # loop
                        ...
                        image_size = int.from_bytes(f1.read(4), "little") # image file size
                        current_position = f1.tell() # get current position
                        f1.seek(current_position + image_size) # seek the next character after the end of the image data of previous character
    ...
    def data_search(self):
        ...
        with open(self.filename, "rb") as f1: # open the Lore file
            with open(self.ref_lore, "rb") as f2: # open the Lore metadata file(.Ref file)
                useroffset = character_index * 4 # get the offset needed for the selected character in the .Ref file
                ...
                image_size = int.from_bytes(f1.read(4), "little") # image file size
                self.image_file = f1.read(image_size) # the image stored within the container file
                ...
                image_data = io.BytesIO(self.image_file)
                pil_image = Image.open(image_data)
                new_size = (400, 400)
                scaled_image = pil_image.resize(new_size, Image.LANCZOS)
                self.image_display = ImageTk.PhotoImage(scaled_image)
                if hasattr(self, 'image_label'):
                    self.image_label.config(image=self.image_display)
                else:
                    self.image_label = ttk.Label(self.root, image=self.image_display)
    ...
    def gui(self):
        ...
        self.slot_combobox.place(x=900, y=10)
        tk.Label(self.root, text="Character Description:").place(x=200, y=600)
        tk.Label(self.root, text="Lore Entries").place(x=820, y=10)
        self.text_info = tk.Text(self.root, wrap = "word", state = "disabled", height = 10, width = 80)
        self.text_info.place(x = 200, y = 650)
        tk.Button(self.root, text="Select Lore File", command=self.file_handling, height=3, width=30).place(x=10, y=10)
